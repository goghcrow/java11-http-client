...
import static jdk11.internal.net.http.hpack.BuffersTestingKit.concat;
...
public final class EncoderTest {
    ...
    private static void encodeAllSplits(Iterable<Consumer<Encoder>> consumers,
        ...
        BuffersTestingKit.forEachSplit(buffer, iterable -> {
            ...
            ByteBuffer data = BuffersTestingKit.concat(copy);
    ...
    public void callSequenceViolations() {
        {   // Hasn't set up a header
            ...
            TestHelper.assertVoidThrows(IllegalStateException.class, () -> e.encode(ByteBuffer.allocate(16)));
        }
        {   // Can't set up header while there's an unfinished encoding
            Encoder e = new Encoder(0);
            e.indexed(32);
            TestHelper.assertVoidThrows(IllegalStateException.class, () -> e.indexed(32));
        }
        {   // Can't setMaxCapacity while there's an unfinished encoding
            Encoder e = new Encoder(0);
            e.indexed(32);
            TestHelper.assertVoidThrows(IllegalStateException.class, () -> e.setMaxCapacity(512));
        }
        {   // Hasn't set up a header
            Encoder e = new Encoder(0);
    ...
    private static void test(Encoder encoder,
        ...
        String actualHexdump = SpecHelper.toHexdump(output);
