...
import static java11.net.http.HttpRequest.BodyPublishers.ofFile;
import static java11.net.http.HttpResponse.BodyHandlers.ofString;
...
public class BasicTest {
    ...
    static void streamTest(boolean secure) throws Exception {
        ...
        dest.toFile().delete();
        CompletableFuture<Path> response = client.sendAsync(req, HttpResponse.BodyHandlers.ofFile(dest))
                .thenApply(resp -> {
    ...
    static void warmup(boolean secure) throws Exception {
        ...
        HttpClient client = getClient();
        HttpRequest req = HttpRequest.newBuilder(uri)
                .POST(HttpRequest.BodyPublishers.ofString(SIMPLE_STRING))
                .build();
    ...
    static void simpleTest(boolean secure, boolean ping) throws Exception {
        ...
        for (int i = 0; i < LOOPS; i++) {
            responses[i] = client.sendAsync(request, HttpResponse.BodyHandlers.ofFile(tempFile()))
