...
import jdk11.internal.net.http.common.SSLTube;
...
import static jdk11.internal.net.http.common.Utils.ServerName;
...
abstract class AbstractAsyncSSLConnection extends HttpConnection
{
    ...
    private static SSLParametersEx createSSLParameters(HttpClientImpl client,
        ...
        SSLParametersEx sslParameters = new SSLParametersEx(Utils.copySSLParameters(sslp));
        ...
        if (!serverName.isLiteral()) {
            ...
            if (name != null && name.length() > 0) {
                sslParameters.setServerNames(Lists.of(new SNIHostName(name)));
            }
        }
    ...
    private static SSLEngineEx createEngine(SSLContext context, String serverName, int port,
                                          SSLParametersEx sslParameters) {
        SSLEngineEx engine = new SSLEngineEx(context.createSSLEngine(serverName, port));
