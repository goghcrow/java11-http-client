...
import java11.lang.System.Logger.Level;
...
import static java11.net.http.HttpResponse.BodySubscribers.discarding;
...
class Http1Response<T> {
    ...
    int fixupContentLen(int clen) {
        if (request.method().equalsIgnoreCase("HEAD") || responseCode == RedirectFilter.HTTP_NOT_MODIFIED) {
            return 0;
        }
        if (clen == -1) {
...
// ============================================================================
    ...
    final class HeadersReader extends Receiver<Http1HeaderParser> {
        ...
        public final void onReadError(Throwable t) {
            t = Utils.wrapWithExtraDetail(t, parser::currentStateMessage);
    ...
    final class BodyReader extends Receiver<ResponseContent.BodyParser> {
        final Consumer<State> onComplete;
        ...
        // Revisit: do we need to support restarting?
        @Override
        final void start(ResponseContent.BodyParser parser) {
            cf = new MinimalFuture<>();
            this.parser = parser;
        }
        ...
        public final void onReadError(Throwable t) {
            if (t instanceof EOFException && bodyParser != null &&
                    bodyParser instanceof ResponseContent.UnknownLengthBodyParser) {
        ...
        @Override
        final void handle(ByteBuffer b,
                          ResponseContent.BodyParser parser,
                          CompletableFuture<State> cf) {
            assert cf != null : "parsing not started";
            assert parser != null : "no parser";
            try {
