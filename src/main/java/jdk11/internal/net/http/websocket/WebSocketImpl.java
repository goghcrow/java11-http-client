...
import jdk11.internal.net.http.common.Demand;
import jdk11.internal.net.http.common.Log;
import jdk11.internal.net.http.common.Logger;
import jdk11.internal.net.http.common.MinimalFuture;
import jdk11.internal.net.http.common.SequentialScheduler;
import jdk11.internal.net.http.common.Utils;
import jdk11.internal.net.http.websocket.OpeningHandshake.Result;
...
import static jdk11.internal.net.http.common.MinimalFuture.failedFuture;
import static jdk11.internal.net.http.websocket.StatusCodes.CLOSED_ABNORMALLY;
import static jdk11.internal.net.http.websocket.StatusCodes.NO_STATUS_CODE;
import static jdk11.internal.net.http.websocket.StatusCodes.isLegalToSendFromClient;
import static jdk11.internal.net.http.websocket.WebSocketImpl.State.BINARY;
import static jdk11.internal.net.http.websocket.WebSocketImpl.State.CLOSE;
import static jdk11.internal.net.http.websocket.WebSocketImpl.State.ERROR;
import static jdk11.internal.net.http.websocket.WebSocketImpl.State.IDLE;
import static jdk11.internal.net.http.websocket.WebSocketImpl.State.OPEN;
import static jdk11.internal.net.http.websocket.WebSocketImpl.State.PING;
import static jdk11.internal.net.http.websocket.WebSocketImpl.State.PONG;
import static jdk11.internal.net.http.websocket.WebSocketImpl.State.TEXT;
import static jdk11.internal.net.http.websocket.WebSocketImpl.State.WAITING;
...
public final class WebSocketImpl implements WebSocket {
    ...
    public static CompletableFuture<WebSocket> newInstanceAsync(BuilderImpl b) {
        Function<Result, WebSocket> newWebSocket = r -> {
            ...
            java11.lang.ref.Reference.reachabilityFence(b);
            return ws;
        };
        OpeningHandshake h;
        try {
    ...
    private boolean tryChangeState(State expectedState, State newState) {
        ...
        // State witness = compareAndExchangeState(expectedState, newState);
        boolean success = false;
    ...
    // 兼容 VarHandle compareAndExchange 返回 Witness

    // 兼容实现1
    State compareAndExchangeState(State expected, State update) {
        while (true) {
            // Witness
            State old = state.get();
            if (old != expected) {
                return old;
            } else if (state.compareAndSet(old, update)) {
                return old;
            }
        }
    }
    ...
    static final class AtomicRef {
        private static final sun.misc.Unsafe UNSAFE = UnsafeAccessor.UNSAFE;
        ...
        static State compareAndExchange(AtomicReference<State> atomicState, State expectedState, State newState) {
            ...
            // XXX
            do {
                witness = UNSAFE.getObjectVolatile(atomicState, VALUE);
            } while (witness == expectedState && !UNSAFE.compareAndSwapObject(atomicState, VALUE, witness, newState));
            return (State) witness;
        }

        static {
            try {
                VALUE = UNSAFE.objectFieldOffset(AtomicReference.class.getDeclaredField("value"));
            } catch (Exception e) {
