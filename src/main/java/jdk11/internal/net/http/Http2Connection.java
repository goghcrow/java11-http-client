...
import jdk11.internal.net.http.common.*;
import jdk11.internal.net.http.common.hack.SSLEngineEx;
import jdk11.internal.net.http.frame.ContinuationFrame;
import jdk11.internal.net.http.frame.DataFrame;
import jdk11.internal.net.http.frame.ErrorFrame;
import jdk11.internal.net.http.frame.FramesDecoder;
import jdk11.internal.net.http.frame.FramesEncoder;
import jdk11.internal.net.http.frame.GoAwayFrame;
import jdk11.internal.net.http.frame.HeaderFrame;
import jdk11.internal.net.http.frame.HeadersFrame;
import jdk11.internal.net.http.frame.Http2Frame;
import jdk11.internal.net.http.frame.MalformedFrame;
import jdk11.internal.net.http.frame.OutgoingHeaders;
import jdk11.internal.net.http.frame.PingFrame;
import jdk11.internal.net.http.frame.PushPromiseFrame;
import jdk11.internal.net.http.frame.ResetFrame;
import jdk11.internal.net.http.frame.SettingsFrame;
import jdk11.internal.net.http.frame.WindowUpdateFrame;
import jdk11.internal.net.http.hpack.Encoder;
import jdk11.internal.net.http.hpack.Decoder;
import jdk11.internal.net.http.hpack.DecodingCallback;
...
class Http2Connection  {
    ...
    private Http2Connection(HttpConnection connection,
        ...
        this.hpackOut = new Encoder(serverSettings.getParameter(SettingsFrame.HEADER_TABLE_SIZE));
        this.hpackIn = new Decoder(clientSettings.getParameter(SettingsFrame.HEADER_TABLE_SIZE));
        if (debugHpack.on()) {
    ...
    final int maxConcurrentClientInitiatedStreams() {
        return serverSettings.getParameter(SettingsFrame.MAX_CONCURRENT_STREAMS);
    }

    final int maxConcurrentServerInitiatedStreams() {
        return clientSettings.getParameter(SettingsFrame.MAX_CONCURRENT_STREAMS);
    }
        ...
        throws IOException
    {
        ...
        if (!frame.getFlag(SettingsFrame.ACK)) {
            int newWindowSize = frame.getParameter(SettingsFrame.INITIAL_WINDOW_SIZE);
            if (newWindowSize != -1) {
                int oldWindowSize = serverSettings.getParameter(SettingsFrame.INITIAL_WINDOW_SIZE);
    ...
    public int getMaxSendFrameSize() {
        int param = serverSettings.getParameter(SettingsFrame.MAX_FRAME_SIZE);
        if (param == -1) {
    ...
    private void sendConnectionPreface() throws IOException {
        ...
        publisher.enqueueUnordered(Lists.of(buf));
        ...
        // 64K -1.
        final int len = windowUpdater.initialWindowSize - SettingsFrame.DEFAULT_INITIAL_WINDOW_SIZE;
        if (len != 0) {
            if (Log.channel()) {
                Log.logChannel("Sending initial connection window update frame: {0} ({1} - {2})",
                        len, windowUpdater.initialWindowSize, SettingsFrame.DEFAULT_INITIAL_WINDOW_SIZE);
            }
     ...
     */
    final class Http2TubeSubscriber implements FlowTube.TubeSubscriber {
    ...
    static class ValidatingHeadersConsumer implements DecodingCallback {
        ...
        private static final Set<String> PSEUDO_HEADERS =
                Sets.of(":authority", ":method", ":path", ":scheme", ":status");
