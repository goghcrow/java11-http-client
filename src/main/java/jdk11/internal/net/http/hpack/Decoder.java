...
public final class Decoder {
    ...
    public Decoder(int capacity) {
        ...
        if (logger.isLoggable(HPACK.Logger.Level.NORMAL)) {
            logger.log(HPACK.Logger.Level.NORMAL, () -> format("new decoder with maximum table size %s",
                                            capacity));
        }
        if (logger.isLoggable(HPACK.Logger.Level.NORMAL)) {
            /* To correlate with logging outside HPACK, knowing
               hashCode/toString is important */
            logger.log(HPACK.Logger.Level.NORMAL, () -> {
    ...
    public void setMaxCapacity(int capacity) {
        if (logger.isLoggable(HPACK.Logger.Level.NORMAL)) {
            logger.log(HPACK.Logger.Level.NORMAL, () -> format("setting maximum table size to %s",
                                            capacity));
        }
        setMaxCapacity0(capacity);
    }
    ...
    public void decode(ByteBuffer headerBlock,
        ...
        if (logger.isLoggable(HPACK.Logger.Level.NORMAL)) {
            logger.log(HPACK.Logger.Level.NORMAL, () -> format("reading %s, end of header block? %s",
        ...
        if (endOfHeaderBlock && state != State.READY) {
            logger.log(HPACK.Logger.Level.NORMAL, () -> format("unexpected end of %s representation",
                                            state));
    ...
    private void resumeReady(ByteBuffer input) {
        ...
        if (logger.isLoggable(HPACK.Logger.Level.EXTRA)) {
            logger.log(HPACK.Logger.Level.EXTRA, () -> format("next binary representation %s (first byte 0x%02x)",
                                           s, b));
        }
        switch (s) {
            case INDEXED:
    ...
    private void resumeIndexed(ByteBuffer input, DecodingCallback action)
        ...
        integerReader.reset();
        if (logger.isLoggable(HPACK.Logger.Level.NORMAL)) {
            logger.log(HPACK.Logger.Level.NORMAL, () -> format("indexed %s", intValue));
        }
        try {
    ...
    private void resumeLiteral(ByteBuffer input, DecodingCallback action)
        ...
        try {
            if (firstValueIndex) {
                if (logger.isLoggable(HPACK.Logger.Level.NORMAL)) {
                    logger.log(HPACK.Logger.Level.NORMAL, () -> format(
    ...
    private void resumeSizeUpdate(ByteBuffer input,
        ...
        intValue = integerReader.get();
        if (logger.isLoggable(HPACK.Logger.Level.NORMAL)) {
            logger.log(HPACK.Logger.Level.NORMAL, () -> format("dynamic table size update %s",
                                            intValue));
        }
        assert intValue >= 0;
