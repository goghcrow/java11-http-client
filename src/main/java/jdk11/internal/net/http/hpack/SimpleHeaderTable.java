...
class SimpleHeaderTable {
    ...
    protected static final List<HeaderField> staticTable = Lists.of(
    ...
    private void put(HeaderField h) {
        if (logger.isLoggable(HPACK.Logger.Level.NORMAL)) {
            logger.log(HPACK.Logger.Level.NORMAL, () -> format("adding ('%s', '%s')",
                                            h.name, h.value));
        }
        int entrySize = sizeOf(h);
        if (logger.isLoggable(HPACK.Logger.Level.EXTRA)) {
            logger.log(HPACK.Logger.Level.EXTRA, () -> format("size of ('%s', '%s') is %s",
                                           h.name, h.value, entrySize));
        }
        while (entrySize > maxSize - size && size != 0) {
            if (logger.isLoggable(HPACK.Logger.Level.EXTRA)) {
                logger.log(HPACK.Logger.Level.EXTRA, () -> format("insufficient space %s, must evict entry",
                                               (maxSize - size)));
            }
            evictEntry();
        }
        if (entrySize > maxSize - size) {
            if (logger.isLoggable(HPACK.Logger.Level.EXTRA)) {
                logger.log(HPACK.Logger.Level.EXTRA, () -> format("not adding ('%s, '%s'), too big",
                                               h.name, h.value));
            }
            return;
        }
        ...
        add(h);
        if (logger.isLoggable(HPACK.Logger.Level.EXTRA)) {
            logger.log(HPACK.Logger.Level.EXTRA, () -> format("('%s, '%s') added", h.name, h.value));
            logger.log(HPACK.Logger.Level.EXTRA, this::toString);
        }
    }
    ...
    HeaderField evictEntry() {
        ...
        this.size -= s;
        if (logger.isLoggable(HPACK.Logger.Level.EXTRA)) {
            logger.log(HPACK.Logger.Level.EXTRA, () -> format("evicted entry ('%s', '%s') of size %s",
                                           f.name, f.value, s));
            logger.log(HPACK.Logger.Level.EXTRA, this::toString);
        }
        return f;
    ...
    static final class CircularBuffer<E> {
        ...
        public void resize(int newCapacity) {
            ...
            int capacity = Utils.pow2Size(newCapacity);
