...
public class Encoder {
    ...
    public Encoder(int maxCapacity) {
        ...
        if (logger.isLoggable(HPACK.Logger.Level.NORMAL)) {
            logger.log(HPACK.Logger.Level.NORMAL, () -> format("new encoder with maximum table size %s",
                                            maxCapacity));
        }
        if (logger.isLoggable(HPACK.Logger.Level.EXTRA)) {
            ...
            logger.log(HPACK.Logger.Level.EXTRA, () -> {
    ...
    public void header(CharSequence name,
        ...
        if (logger.isLoggable(HPACK.Logger.Level.NORMAL)) {
            logger.log(HPACK.Logger.Level.NORMAL, () -> format("encoding ('%s', '%s'), sensitive: %s",
    ...
    private void setMaxCapacity0(int capacity) {
        ...
        if (logger.isLoggable(HPACK.Logger.Level.NORMAL)) {
            logger.log(HPACK.Logger.Level.NORMAL, () -> format("actual maximum table size will be %s",
                                            calculated));
        }
    ...
    public final boolean encode(ByteBuffer headerBlock) {
        ...
        if (logger.isLoggable(HPACK.Logger.Level.EXTRA)) {
            logger.log(HPACK.Logger.Level.EXTRA, () -> format("writing to %s", headerBlock));
        }
    ...
    protected final void indexed(int index) throws IndexOutOfBoundsException {
        checkEncoding();
        if (logger.isLoggable(HPACK.Logger.Level.EXTRA)) {
            logger.log(HPACK.Logger.Level.EXTRA, () -> format("indexed %s", index));
        }
        encoding = true;
    ...
    protected final void literal(int index,
        ...
        if (logger.isLoggable(HPACK.Logger.Level.EXTRA)) {
            logger.log(HPACK.Logger.Level.EXTRA, () -> format(
    ...
    protected final void sizeUpdate(int capacity)
            throws IllegalArgumentException {
        if (logger.isLoggable(HPACK.Logger.Level.EXTRA)) {
            logger.log(HPACK.Logger.Level.EXTRA, () -> format("dynamic table size update %s",
                                           capacity));
        }
        checkEncoding();
