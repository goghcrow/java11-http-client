...
import jdk11.internal.net.http.common.FlowTube;
import jdk11.internal.net.http.common.Logger;
import jdk11.internal.net.http.common.Utils;
import jdk11.internal.net.http.websocket.RawChannel;
...
//            Utils.ASSERTIONSENABLED  && Utils.DEBUG_WS ? Cleaner.create() : null;
    ...
    RawChannelTube(HttpConnection connection,
        ...
        if (Utils.ASSERTIONSENABLED && Utils.DEBUG_WS) {
            ...
            // 反正这里只是为了调试，直接用 Cleaner 创建好了
            Cleaner.create(this, new CleanupChecker(closed, debug));
        }
    }
    ...
    static final class CleanupChecker implements Runnable {
        ...
        final java11.lang.System.Logger debug;
        CleanupChecker(AtomicBoolean closed, java11.lang.System.Logger debug) {
            this.closed = closed;
            this.debug = debug;
        }
    ...
    private static List<ByteBuffer> copy(ByteBuffer[] src, int offset, int len) {
        ...
        if (count == 1) return Lists.of(Utils.copy(src[offset]));
        if (count == 2) return Lists.of(Utils.copy(src[offset]), Utils.copy(src[offset+1]));
