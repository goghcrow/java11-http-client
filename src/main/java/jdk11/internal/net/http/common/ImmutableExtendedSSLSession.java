...
public class ImmutableExtendedSSLSession extends ExtendedSSLSession {
    ...
    public List<byte[]> getStatusResponses() {
        if (HackUtils.isJava8()) {
            throw new AssertionError();
        } else {
            // return delegate.getStatusResponses();
            try {
                //noinspection unchecked
                return ((List<byte[]>) GET_STATUE_RESPONSES.invoke(delegate));
    ...
    private static Method GET_STATUE_RESPONSES = null;

    static {
        if (!HackUtils.isJava8()) {
            try {
                GET_STATUE_RESPONSES = ExtendedSSLSession.class.getDeclaredMethod("getStatusResponses");
                GET_STATUE_RESPONSES.setAccessible(true);
